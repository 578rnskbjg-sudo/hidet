    ...
    for head_size in [32, 64, 96, 128, 160, 192, 256]:
        for warps in [2, 4, 8]:
            for bn in [64, 128]:
                ...
                c = TensorLayout(((4, 8), (2, 2)), ((2, 8), (1, 64)))
                mma_atom = MmaAtom("warp", (8, 16, 16), a, b, c, c, (1, 1))
                if head_size % (16 * warps) != 0:
                    continue
                # a condition to filter invalid kernel
                c_regs = head_size // (16 * warps) * 4
                if c_regs < 8:
                    continue
                warp_in_threadblock = Level(
                    "warp",
        ...
        def func(
            ...
            for ho in range((head_size + bk - 1) // bk):
                for hi in range(h_tile_max):
                    ...
                    copy(
                        auto_copy(),
                        txSq[:, :, h_tile_next, smem_pipe_read],
                        txrq[:, :, (hi + 1) % 2],
                    )
                    copy(
                        auto_copy(),
                        txSk[:, :, h_tile_next, smem_pipe_read],
                        txrk[:, :, (hi + 1) % 2],
                    )
                    if hi == 0:
            ...
            if pid_n < num_pid_n - 1:
                tg_qk_max = tensor_view(
                    imm[batch_idx, head_idx, pid_n, pid_m * bm :],
                    TensorLayout((bm, bn), (1, 0)),
                    "global",
                )
...
def flash_decoding_v3(
    ...
    seqlen_k,
    num_heads_k,
    num_seqk_parallel_parts: int = 128,
):
    from hidet.lang.types import u32, i32, f16, f32
    from hidet.lang import attrs
    from hidet.lang import shared_tensor, register_tensor
        ...
        def func(
            ...
            for no in range(no_size):
                ...
                if no < no_size - 1:
                    tg_k1 = tensor_view(
                        k[
                            batch_idx,
                            seqk_part * seqk_partition + (no + 1) * bn :,
                            head_idx,
                            :,
                        ],
                        TensorLayout((bn, head_size), (num_heads_k * head_size, 1)),
            ...
            if seqk_part > 0:
                ...
                tg_oi = tensor_view(
                    ...
                    TensorLayout(
                        (bm, head_size),
                        (num_heads * num_seqk_parallel_parts * head_size, 1),
                    ),
                    "global",
                )
                tg_li = tensor_view(
                    li[batch_idx, head_idx, seqk_part, pid_m * bm :],
                    TensorLayout((bm, bn), (1, 0)),
                    "global",
                )
                tg_mi = tensor_view(
                    mi[batch_idx, head_idx, seqk_part, pid_m * bm :],
                    TensorLayout((bm, bn), (1, 0)),
                    "global",
                )
                ...
                for i in range(num_seqk_parallel_parts - 1):
                    ...
                    tg_li = tensor_view(
                        li[batch_idx, head_idx, i + 1, pid_m * bm :],
                        TensorLayout((bm, bn), (1, 0)),
                        "global",
                    )
                    tg_mi = tensor_view(
                        mi[batch_idx, head_idx, i + 1, pid_m * bm :],
                        TensorLayout((bm, bn), (1, 0)),
                        "global",
                    )
                    txgx_oi = partition_dst(tg_oi, auto_copy())
...
def test_flash_decoding(batch_size, num_heads, num_heads_k, head_size, seqlen_q, seqlen_k):
    ...
    def fn():
        ...
        li = torch.empty(
            (batch_size, num_heads, num_seqk_parallel_parts, seqlen_q),
            dtype=torch.float32,
            device="cuda",
        )
        mi = torch.empty(
        ...
        oi = torch.empty(
            (batch_size, seqlen_q, num_heads, num_seqk_parallel_parts, head_size),
            dtype=torch.float16,
            device="cuda",
        )
...
class FlashDecoding:
    def __init__(self, batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k, sm_ver):
    ...
    @tune.space(
        ...
        num_seqk_parallel_parts=[2, 4, 8, 16],
    )
    def _candidates(self, tiled_mma_pairs: Tuple[TiledMma, TiledMma], num_seqk_parallel_parts: int):
        ...
        batch_size, _, num_heads, head_size, seqlen_k, num_heads_k = (
            self.batch_size,
            self.seqlen_q,
            self.num_heads,
            self.head_size,
        ...
        q_shape, _ = tiled_mma_qk.a_tv_layout()
        k_shape, _ = tiled_mma_qk.b_tv_layout()
        qk_shape, qk_tv_layout = tiled_mma_qk.c_tv_layout()
        ...
        qk_shape, _ = tiled_mma_o.a_tv_layout()
        o_shape, _ = tiled_mma_o.c_tv_layout()
        _, inst_n = qk_shape
        ...
        tune.check(seqlen_k % num_seqk_parallel_parts == 0)
        seqk_partition = seqlen_k // num_seqk_parallel_parts
        tune.check(seqk_partition % bn == 0)

        with hidet.script_module() as script_module:

            @hidet.script
            def func(
                q: f16[batch_size, num_heads, head_size],
                k: f16[batch_size, seqlen_k, num_heads_k, head_size],
                v: f16[batch_size, seqlen_k, num_heads_k, head_size],
                o: f16[batch_size, num_heads, head_size],
                lock: i32[batch_size, num_heads],
                li: f32[batch_size, num_heads, num_seqk_parallel_parts, bm],
                mi: f32[batch_size, num_heads, num_seqk_parallel_parts, bm],
                oi: f16[batch_size, num_heads, num_seqk_parallel_parts, head_size],
            ):
                attrs.func_kind = "cuda_kernel"
                attrs.cuda.block_dim = threads
                attrs.cuda.grid_dim = bs * num_seqk_parallel_parts
                ...
                batch_idx = pid // num_heads
                head_idx = pid % num_heads
                msk_q = mask(auto_copy(), [i32(1), i32(head_size)])
                ...
                ts_q = make_tensor("float16", TensorLayout((bm, head_size), (head_size, 1)), "shared")
                txsq = partition_dst(ts_q, auto_copy())
                ts_k = make_tensor("float16", TensorLayout((head_size, bn), (1, head_size)), "shared")
                txsk = partition_dst(ts_k, auto_copy())
                ts_v = make_tensor("float16", TensorLayout((head_size, bn), (1, head_size)), "shared")
                txsv = partition_dst(ts_v, auto_copy())
                ts_qk = make_tensor("float16", layout_auto((bm, bn)), "shared")
                ...
                copy(auto_copy((bm, head_size)), txgq, txsq, msk_q)
                ...
                lc = ~lock[batch_idx, head_idx]

                msk_o = mask(auto_copy(), [i32(1), i32(head_size)])

                if seqk_part > 0:
                    tr_o_f16 = cast(tr_o, f16)
                    tr_O = rearrange(tr_o_f16, auto_layout, "register")
                    msk_o_cont = mask(auto_copy(), [i32(1), i32(head_size)])
                    tg_oi = tensor_view(
                        oi[batch_idx, head_idx, seqk_part, 0:],
                        TensorLayout(
                            (bm, head_size),
                            (num_heads * num_seqk_parallel_parts * head_size, 1),
                        ),
                    ...
                    tg_li = tensor_view(
                        li[batch_idx, head_idx, seqk_part, :],
                        TensorLayout((bm, bn), (1, 0)),
                        "global",
                    )
                    tg_mi = tensor_view(
                        mi[batch_idx, head_idx, seqk_part, :],
                        TensorLayout((bm, bn), (1, 0)),
                        "global",
                    )
                    txrx_o = partition_src(tr_O, auto_copy())
                    txgx_oi = partition_dst(tg_oi, auto_copy())
                    copy(auto_copy((bm, head_size)), txrx_o, txgx_oi, msk_o_cont)
                    txrqk_max = partition_src(tr_qk_max, auto_copy())
                    txgqk_max = partition_dst(tg_li, auto_copy())
                    copy(auto_copy((bm, bn)), txrqk_max, txgqk_max)
                    txrqk_sum = partition_src(tr_qk_sum, auto_copy())
                    ...
                    for i in range(num_seqk_parallel_parts - 1):
                        ...
                        tg_li = tensor_view(
                            li[batch_idx, head_idx, i + 1, :],
                            TensorLayout((bm, bn), (1, 0)),
                            "global",
                        )
                        tg_mi = tensor_view(
                            mi[batch_idx, head_idx, i + 1, :],
                            TensorLayout((bm, bn), (1, 0)),
                            "global",
                        )
                        txgx_oi = partition_dst(tg_oi, auto_copy())
                        txrx_oi = partition_src(tr_oi, auto_copy())
                        copy(auto_copy((bm, head_size)), txgx_oi, txrx_oi, msk_o)
                        txgli = partition_src(tg_li, auto_copy())
                        txrli = partition_dst(tr_li, auto_copy())
                        copy(auto_copy((bm, bn)), txgli, txrli)
                        txgmi = partition_src(tg_mi, auto_copy())
                        txrmi = partition_dst(tr_mi, auto_copy())
                        copy(auto_copy((bm, bn)), txgmi, txrmi)
                        scale = exp2((tr_qk_max - elementwise_max(tr_qk_max, tr_li)) * sm_scale)
                        tr_qk_max = elementwise_max(tr_qk_max, tr_li)
                        scale1 = exp2(tr_li * sm_scale - tr_qk_max * sm_scale)
                        tr_qk_sum = tr_qk_sum * scale
                        tr_mi = tr_mi * scale1
                        alpha = tr_qk_sum / (tr_qk_sum + tr_mi)
                        tr_qk_sum = tr_qk_sum + tr_mi
                    ...
                    msk_o_cont = mask(auto_copy(), [i32(1), i32(head_size)])
                    tg_o = tensor_view(
                        o[batch_idx, head_idx, 0:],
                        TensorLayout((bm, head_size), (num_heads * head_size, 1)),
                        "global",
                    )
                    txrx_o = partition_src(tr_O, auto_copy())
                    txgx_o = partition_dst(tg_o, auto_copy())
                    copy(auto_copy((bm, head_size)), txrx_o, txgx_o, msk_o_cont)

        return script_module.ir_module()


def flash_decoding_interface(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k, sm_ver):
    flashdecode = FlashDecoding(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k, sm_ver)
    return flashdecode.build()


def main(batch_sizes: int, num_heads: int, num_heads_k: int, head_size: int, seqlen_q: int, seqlen_k: int, sm_ver: int, cand: int = None):
    artifacts = flash_decoding_interface(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k, sm_ver)
    q, k, v, o = data(
        batch_size,
        seqlen_q,
        num_heads,
    ...
    for i, (m, func) in enumerate(artifacts):
        ...
        num_seqk_parallel_parts = m._tuning_kwargs["num_seqk_parallel_parts"]

        def fn():
            bm = 8
            lock = torch.zeros((batch_size, num_heads), dtype=torch.int32, device="cuda")
            li = torch.empty(
                (batch_size, num_heads, num_seqk_parallel_parts, bm),
                dtype=torch.float32,
                device="cuda",
            )
            mi = torch.empty(
            ...
            oi = torch.empty(
                (batch_size, num_heads, num_seqk_parallel_parts, head_size),
                dtype=torch.float16,
                device="cuda",
            )
            func(q, k, v, o, lock, li, mi, oi)
    ...
    num_seqk_parallel_parts = best_module._tuning_kwargs["num_seqk_parallel_parts"]
    print(num_seqk_parallel_parts)
    ...
    import flashinfer
    q = q.reshape(num_heads, head_size)
    k = k.reshape(seqlen_k, num_heads_k, head_size)
    v = v.reshape(seqlen_k, num_heads_k, head_size)
    ...
    out2 = fn()
    out2 = out2.reshape(batch_size, seqlen_q, num_heads, head_size)
    np.testing.assert_allclose(actual=o.cpu().numpy(), desired=out2.cpu().numpy(), rtol=1e-2)
    
    from xformers.ops.fmha.triton_splitk import FwOp, FwOp_Map
    from xformers.ops import fmha
    FwOp.AUTOTUNE = False

    q = q.reshape(batch_size, 1, num_heads, head_size)
    k = k.reshape(batch_size, seqlen_k, num_heads_k, head_size)
    v = v.reshape(batch_size, seqlen_k, num_heads_k, head_size)

    @torch.no_grad
    def fn3():
        out = fmha.memory_efficient_attention_forward(q, k, v, op=FwOp)
        return out
        #lse1 = lse1[..., :num_heads]

        #attn_chunks = [out1, out1]
        #lse_chunks = [lse1, lse1]
        #out, lse = fmha.merge_attentions(out1, lse1, write_lse=False)  # type: ignore
        #return out
    out3 = fn3()
    #mean, _, _ = bench(fn3, ())
    ...
    np.testing.assert_allclose(actual=o.cpu().numpy(), desired=out3.cpu().numpy(), rtol=1e-2)

    return mean_hexcute, mean_flash_atten, mean_flash_infer, mean_triton
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark the performance of the attention decoding layer provided by CUTE and flash-attention"
    )
    parser.add_argument("--batch-size", type=int, default=1)
    parser.add_argument("--num-heads", type=int, default=32)
    parser.add_argument("--num-heads-k", type=int, default=32)
    ...
    if args.output is None:
        mean_hexcute, mean_flash_atten, mean_flash_infer, mean_triton = main(batch_size, num_heads, num_heads_k, head_size, seqlen_q, seqlen_k, sm_ver, args.cand)
        exit()
