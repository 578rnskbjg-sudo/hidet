...
def gemm_multi_buffer(m, n, k):
    from hidet.lang.types import u32, i32, f64
    ...
    with hidet.script_module() as script_module:
        ...
        @hidet.script
        def func(a: f64[m, k], b: f64[k, n], c: f64[m, n]):
            ...
            ts_a = make_tensor("float64", layout_auto((bm, bk, stages)), "shared")
            ts_b = make_tensor("float64", layout_auto((bn, bk, stages)), "shared")

            tr_a = make_tensor("float64", layout_auto((bm, inst_k * 2)), "register")
            tr_b = make_tensor("float64", layout_auto((bn, inst_k * 2)), "register")
            tr_c = make_tensor("float64", auto_layout, "register")
            fill(tr_c, 0.0)
...
def test_gemm_multi_buffer_f64(m, n, k):
    hidet.option.cache_dir("./demo_gemm_f64")
    hidet.option.search_space(2)
    hidet.option.debug_cache_tuning()
    hidet.option.save_lower_ir(True)

    func = gemm_multi_buffer(m, n, k)
    return
    a, b, c = data(m, n, k, dtype="float64", return_hidet=True)
    mean, min_lat, max_lat = bench(func, (a, b, c))
    print("time={:.3f} ms, performance={:.3f} TFLOPS".format(mean, 2.0 * (m * n * k) / (1e9 * mean)))
...
if __name__ == "__main__":
    ...
    test_gemm_multi_buffer_f64(4096, 4096, 4096)
