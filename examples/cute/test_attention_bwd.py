...
def repeat_policy(block_m: int, block_n: int, warps: int):
    ...
    M = math.floor(math.sqrt(repeat_mn))
    while repeat_mn % M != 0 or block_m % (inst_m * M) != 0 or block_n % (inst_n * repeat_mn // M) != 0:
        M -= 1
        if M == 0:
            break
    if M == 0:
        ...
        for warps in [8]:
            for bc in [32, 64, 128]:
                for br in [32, 64, 128]:
                    ...
                    if packed is None:
                        continue
                    warp_m, warp_n, repeat_m, repeat_n = packed
                    a = TensorLayout(((4, 8), (2, 2, 2)), ((32, 1), (16, 8, 128)))
                    b = TensorLayout(((4, 8), (2, 2)), ((16, 1), (8, 64)))
                    c = TensorLayout(((4, 8), (2, 2)), ((32, 1), (16, 8)))
                    mma_atom = MmaAtom("warp", (16, 8, 16), a, b, c, c, (1, 2))
...
class FlashAttentionBackward:
    def __init__(self, batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k):
    ...
    def _candidates(
        ...
        tune.check(smem_estimated <= 99000)
...
def flash_attention_bwd_interface(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k):
    flashattn = FlashAttentionBackward(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k)
    return flashattn.modules()


if __name__ == "__main__":
    hidet.option.cache_dir("attn_bwd")
    ...
    func_preprocess = flash_attention_bwd_preprocess(batch_size, seqlen_q, num_heads, head_size)
    modules = flash_attention_bwd_interface(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k)
    ...
    for i, mod in enumerate(modules):
        ...
        def fn():
            func_preprocess(o, do, dp_sum, dq)
            func(q, k, v, o, do, dq, dk, dv, lse, m, dp_sum)
