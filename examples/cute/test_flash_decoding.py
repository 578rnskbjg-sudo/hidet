...
def flash_decoding_interface(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k):
    flashdecode = FlashDecoding(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k)
    return flashdecode.modules()


if __name__ == "__main__":
    hidet.option.cache_dir("decode")
    ...
    modules = flash_decoding_interface(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k)
    q, k, v, o = data(
        batch_size,
        seqlen_q,
        num_heads,
    ...
    def fn():
        lock = torch.empty((batch_size, num_heads), dtype=torch.int32, device="cuda")
        li = torch.empty(
            (batch_size, num_heads, num_seqk_parallel_parts),
            dtype=torch.float32,
            device="cuda",
        )
        mi = torch.empty(
