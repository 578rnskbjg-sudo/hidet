...
class GemmQuantModule:
    def __init__(self, gemm_quant: IRModule, q: Tuple[IRModule, ...] = None, dq: Tuple[IRModule, ...] = None, m2 = None, m1 = None):
...
def gemm_quant_module(gemm_quant: IRModule, q: Tuple[IRModule, ...] = None, dq: Tuple[IRModule, ...] = None, m2 = None, m1 = None):
    return GemmQuantModule(gemm_quant, q, dq, m2, m1)
...
def data_intxb(M, N, K, dtype="int8", device="cuda", return_hidet=False, group_size=128):
    dtype = getattr(torch, dtype)
    lo = -2
    hi = 2
    a = torch.randint(low=lo, high=hi, size=(M, K), dtype=dtype, device=device)
