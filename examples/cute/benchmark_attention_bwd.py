...
_tiled_mmas: List[Tuple[TiledMma, TiledMma]] = []


def repeat_policy(block_m: int, block_n: int, warps: int):
    # smallest tile
    inst_m, inst_n = 16, 16
    total = block_m * block_n
    if total % (warps * inst_m * inst_n) != 0:
        return []
    repeat_mn = total // (warps * inst_m * inst_n)
    # I guess the closest two number
    import math

    results = []
    end = math.floor(math.sqrt(repeat_mn))
    M = end
    while M <= repeat_mn:
        if repeat_mn % M != 0 or block_m % (inst_m * M) != 0 or block_n % (inst_n * repeat_mn // M) != 0:
            M += 1
        else:
            N = repeat_mn // M
            warp_m = block_m // (inst_m * M)
            warp_n = block_n // (inst_n * N)
            results.append((warp_m, warp_n, M, N))
            return results
            M += 1
    return results 
    #results = []
    #if warps == 4:
    #    warp_arrangements = [(2, 2)]
    #else:
    #    warp_arrangements = [(2, 4), (4, 2)]
    #for warp_m, warp_n in warp_arrangements:
    #    if block_m % (warp_m * inst_m) != 0 or block_n % (warp_n * inst_n) != 0:
    #        continue
    #    M = block_m // (warp_m * inst_m)
    #    N = block_n // (warp_n * inst_n)
    #    results.append((warp_m, warp_n, M, N))
    #return results
...
def register_tiled_mma():
    for head_size in [32, 64, 96, 128, 160, 192, 256]:
        for warps in [4, 8]:
            for bc in [32, 64, 128]:
                for br in [32, 64, 128]:
                    # bc = 64
                    # br = 64
                    # head_size = 128
                    packed = repeat_policy(bc, br, warps)
                    if len(packed) == 0:
                        continue
                    tiled_mma_qks = []
                    for item in packed:
                        warp_m, warp_n, repeat_m, repeat_n = item
                        a = TensorLayout(((4, 8), (2, 2, 2)), ((32, 1), (16, 8, 128)))
                        b = TensorLayout(((4, 8), (2, 2)), ((16, 1), (8, 64)))
                        c = TensorLayout(((4, 8), (2, 2)), ((32, 1), (16, 8)))
                        mma_atom = MmaAtom("warp", (16, 8, 16), a, b, c, c, (1, 2))
                        warp_in_threadblock = Level(
                            ...
                            "thread_block",
                            (warp_m, warp_n),
                            TensorLayout((warp_m, warp_n)),
                            (repeat_m, repeat_n),
                        )
                        tiled_mma_qks.append(TiledMma(mma_atom, [warp_in_threadblock]))

                    # bc = 64
                    # head_size = 128
                    packed = repeat_policy(bc, head_size, warps)
                    if len(packed) == 0:
                        continue
                    tiled_mma_pdos = []
                    for item in packed:
                        ...
                        tiled_mma_pdos.append(TiledMma(mma_atom, [warp_in_threadblock]))
                    ...
                    for item in packed:
                        ...
                        tiled_mma_dsqs.append(TiledMma(mma_atom, [warp_in_threadblock]))

                    # br = 64
                    # head_size = 128
                    packed = repeat_policy(br, head_size, warps)
                    if len(packed) == 0:
                        continue
                    tiled_mma_dsks = []
                    for item in packed:
                        ...
                        tiled_mma_dsks.append(TiledMma(mma_atom, [warp_in_threadblock]))

                    import itertools
                    for tiled_mma_qk, tiled_mma_pdo, tiled_mma_dsq, tiled_mma_dsk in itertools.product(tiled_mma_qks, tiled_mma_pdos, tiled_mma_dsqs, tiled_mma_dsks):
                        _tiled_mmas.append((tiled_mma_qk, tiled_mma_pdo, tiled_mma_dsq, tiled_mma_dsk))
        ...
        def func(
            ...
            for j in grid(tc):
                ...
                for i in grid(tr):
                    ...
                    tg_l = tensor_view(
                        l[batch_idx, head_idx, i * br :], TensorLayout((bc, br), (0, 1)), "global"
                    )
                    tg_m = tensor_view(
                        m[batch_idx, head_idx, i * br :], TensorLayout((bc, br), (0, 1)), "global"
                    )
    ...
    with hidet.script_module() as script_module:
        ...
        def func(
            ...
            if seqk_part == 0:
                ...
                for i in grid(tr):
                    tr_dq = make_tensor(
                        "float16", layout_auto((num_parallel_seqk_parts, br, head_size)), "register"
                    )
                    fill(tr_dq, 0.0)
                    tg_dq = tensor_view(
...
def flash_attention_v4_bwd(
    ...
    with hidet.script_module() as script_module:
        ...
        def func(
            ...
            ts_q = make_tensor(
                "float16", TensorLayout((br, head_size, 2), (head_size, 1, br * head_size)), "shared"
            )
            txSq = partition_src(ts_q, auto_copy())
            txsq = partition_dst(ts_q, auto_copy())
            ...
            for j in grid(tc):
                ...
                tg_dp_sum = tensor_view(
                    dp_sum[batch_idx, head_idx, 0:], TensorLayout((bc, seqlen_q), (0, 1)), "global"
                )
...
def data(
    ...
    hi = 2
    q = torch.randint(
        low=lo, high=hi, size=(batch_size, seqlen_q, num_heads, head_size), dtype=dtype, device=device
    )
    k = torch.randint(
        low=lo, high=hi, size=(batch_size, seqlen_k, num_heads_k, head_size), dtype=dtype, device=device
    )
    v = torch.randint(
        low=lo, high=hi, size=(batch_size, seqlen_k, num_heads_k, head_size), dtype=dtype, device=device
    )
    ...
    do = torch.randint(
        low=0, high=hi, size=(batch_size, seqlen_q, num_heads, head_size), dtype=dtype, device=device
    )
    lse = torch.randint(
...
def test_v4(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k, num_parallel_seqk_parts):
    ...
    # from flash_attn_interface import _flash_attn_backward import flash_attn_bwd
    ...
    def fn2(dq1, dk1, dv1):
        dq1, dk1, dv1, softmax_d = flash_attn_cuda.bwd(
            do, q, k, v, o, lse, dq1, dk1, dv1, None, 0.0, 1.0, False, -1, -1, 0.0, False, None, None
        )
        return dq1, dk1, dv1
...
class FlashAttentionBackward:
    ...
    def modules(self):
        return tune.extract_ir_modules(self._candidates_v_in_regs) + tune.extract_ir_modules(self._candidates_v_in_smem)
    ...
    @tune.space(
        2,
        tiled_mmas=_tiled_mmas,
        num_parallel_seqk_parts=[-1, 2, 4, 8, 16, 32],
    )
    def _candidates_v_in_regs(
        self, tiled_mmas: Tuple[TiledMma, TiledMma, TiledMma, TiledMma], num_parallel_seqk_parts: int
    ):
        from hidet.lang.types import u32, i32, f16, f32
        from hidet.lang import attrs
        from hidet.lang import shared_tensor, register_tensor
        ...
        tiled_mma_qk, tiled_mma_pdo, tiled_mma_dsq, tiled_mma_dsk = tiled_mmas
        ...
        qk_t, _ = canonicalize(qk_tv_layout)
        ...
        p_shape, _ = tiled_mma_pdo.a_tv_layout()
        dv_shape, _ = tiled_mma_pdo.c_tv_layout()

        _, inst_r = p_shape
        _, head_size_ = dv_shape
        tune.check(head_size_ == head_size)

        dst_shape, _ = tiled_mma_dsk.a_tv_layout()
        ...
        smem_estimated = f16.nbytes * (br * head_size * 2 + bc * head_size + br * head_size + bc * br * 2)
        ...
        tune.check(smem_estimated <= max_smem)
       
        tune.check(num_parallel_seqk_parts == -1)
        seqk_partition = bc
        if num_parallel_seqk_parts == -1:
            prop = hidet.cuda.properties()
            num_sms = prop.multiProcessorCount
            num_parallel_seqk_parts = cdiv(seqlen_k, bc)
            tc = cdiv(seqlen_k, num_parallel_seqk_parts * bc)
        else:
            tune.check(seqk_partition * num_parallel_seqk_parts <= seqlen_k) 
            while seqk_partition * num_parallel_seqk_parts < seqlen_k:
                seqk_partition += seqk_partition
            tc = cdiv(seqk_partition, bc)
        tune.check(seqlen_k % seqk_partition == 0)
        tr = cdiv(seqlen_q, br)
        print(f"{bc}, {br}, {head_size}")
        ...
        with hidet.script_module() as script_module:
            ...
            def func(
                ...
                for j in grid(tc):
                    ...
                    seqk_start = seqk_part * seqk_partition + j * bc
                    tg_k = tensor_view(
                        k[batch_idx, seqk_start :, head_idx, :],
                        TensorLayout((bc, head_size), (num_heads_k * head_size, 1)),
                        "global",
                    )
                    txgk = partition_src(tg_k, auto_copy())
                    tg_v = tensor_view(
                        v[batch_idx, seqk_start :, head_idx, :],
                        TensorLayout((bc, head_size), (num_heads_k * head_size, 1)),
                        "global",
                    )
                    txgv = partition_src(tg_v, auto_copy())
                    ...
                    li = make_tensor("float32", layout_auto((bc, br), (0, 1)), "register")
                    tg_l = tensor_view(
                        l[batch_idx, head_idx, :], TensorLayout((bc, seqlen_q), (0, 1)), "global"
                    )
                    txgli = partition_src(tg_l, auto_copy())
                    txrli = partition_dst(li, auto_copy())
                    copy(auto_copy((bc, br)), txgli[:, :, 0], txrli)
                    ...
                    for i in grid(tr):
                        ...
                        p = exp2(tr_qk * sm_scale)
                        
                        # compute v*do^T
                        tr_dp = make_tensor("float32", auto_layout, "register")
                        fill(tr_dp, 0.0)
                        tr_dp = tr_dp - tr_dp_sum
                        ...
                        copy(auto_copy(), txSp[:, :, 0], txrp1[:, :, 0])
                        copy(auto_copy(), txSdo[:, :, 0], txrdo[:, :, 0])
                        for ri in grid(r_tile_max, attrs="u+"):
                            if ri < r_tile_max - 1:
                                copy(auto_copy(), txSp[:, :, ri + 1], txrp1[:, :, (ri + 1) % 2])
                                copy(auto_copy(), txSdo[:, :, ri + 1], txrdo[:, :, (ri + 1) % 2])
                            mma(tiled_mma_pdo, tr_dv, txrp1[:, :, ri % 2], txrdo[:, :, ri % 2], tr_dv)
                        ...
                        copy(auto_copy(), txSdst[:, :, 0], txrdst[:, :, 0])
                        copy(auto_copy(), txSkt[:, :, 0], txrkt[:, :, 0])
                        for ci in grid(c_tile_max, attrs="u+"):
                            if ci < c_tile_max - 1:
                                copy(auto_copy(), txSdst[:, :, ci + 1], txrdst[:, :, (ci + 1) % 2])
                                copy(auto_copy(), txSkt[:, :, ci + 1], txrkt[:, :, (ci + 1) % 2])
                            mma(tiled_mma_dsk, tr_dq, txrdst[:, :, ci % 2], txrkt[:, :, ci % 2], tr_dq)
                        ...
                        if i < tr - 1:
                            copy(auto_copy((bc, br)), txgli[:, :, i + 1], txrli)
                    ...
                    seqk_start = seqk_part * seqk_partition + j * bc
                    tg_dK = tensor_view(
                        dk[batch_idx, seqk_start :, head_idx, :],
                        TensorLayout((bc, head_size), (num_heads_k * head_size, 1)),
                        "global",
                    )
                    tg_dV = tensor_view(
                        dv[batch_idx, seqk_start :, head_idx, :],
                        TensorLayout((bc, head_size), (num_heads_k * head_size, 1)),
                        "global",
                    )
                    txgdk = partition_dst(tg_dK, auto_copy())
    ...
    def _candidates_v_in_smem(
        ...
        smem_estimated = f16.nbytes * (br * head_size * 2 + bc * head_size + bc * head_size + br * head_size + bc * br * 2)
        smem_limits = {70: 96000, 72: 96000, 75: 64000, 80: 163000, 86: 99000, 87: 163000, 89: 99000, 90: 227000}
        max_smem = 99000 if compute_capability > 90 else smem_limits[compute_capability]
        tune.check(smem_estimated <= max_smem)
        sm_scale = 1.44269504
        ...
        if num_parallel_seqk_parts == -1:
            prop = hidet.cuda.properties()
            num_sms = prop.multiProcessorCount
            num_parallel_seqk_parts = cdiv(num_sms, bs)
            num_parallel_seqk_parts = cdiv(seqlen_k, bc)
            tc = cdiv(seqlen_k, num_parallel_seqk_parts * bc)
        else:
            tune.check(seqk_partition * num_parallel_seqk_parts <= seqlen_k) 
...
def flash_attention_bwd_interface(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k, sm_ver):
    flashattn = FlashAttentionBackward(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k, sm_ver)
...
def main(batch_sizes: int, num_heads: int, num_heads_k: int, head_size: int, seqlen_q: int, seqlen_k: int, sm_ver: int, cand: int = None):
    func_preprocess = flash_attention_bwd_preprocess(batch_size, seqlen_q, num_heads, head_size)
    artifacts = flash_attention_bwd_interface(batch_size, seqlen_q, num_heads, head_size, seqlen_k, num_heads_k, sm_ver)
    ...
    for i, (mod, func) in enumerate(artifacts):
        ...
        tiled_mma_qk, tiled_mma_pdo, tiled_mma_dsq, tiled_mma_dsk = mod._tuning_kwargs["tiled_mmas"]

        k_shape, _ = tiled_mma_qk.a_tv_layout()
        q_shape, _ = tiled_mma_qk.b_tv_layout()

        br, _ = q_shape
        bc, _ = k_shape

        num_parallel_seqk_parts = mod._tuning_kwargs["num_parallel_seqk_parts"]

        def fn():
            func_preprocess(o, do, dp_sum, dq)
            func(q, k, v, o, do, dq, dk, dv, lse, dp_sum)

        from hidet.utils.benchmark import do_bench

        #mean, min_lat, max_lat = bench(fn, ())
        mean = do_bench(fn, percentiles=None)
        print("br={}, bc={}".format(br, bc))
        print("time={:.3f} ms, performance={:.3f} TFLOPS".format(mean, flops / (1e9 * mean)))
        print("time={:.3f} ms, bandwidth={:.3f} GB/s".format(mean, memory / (1e6 * mean)))
    ...
    # check valid
    num_parallel_seqk_parts = best_module._tuning_kwargs["num_parallel_seqk_parts"]
    func = best_func
    func_preprocess(o, do, dp_sum, dq)
    func(q, k, v, o, do, dq, dk, dv, lse, dp_sum)
    ...
    print(num_parallel_seqk_parts)
    return mean_hexcute, mean_flash_atten
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark the performance of the standard attention backward layer provided by CUTE and flash-attention"
    )
    ...
    if args.output is None:
        mean_hexcute, mean_flash_atten = main(batch_size, num_heads, num_heads_k, head_size, seqlen_q, seqlen_k, sm_ver, args.cand)
        exit()

    from tabulate import tabulate

    records = []
    headers = ["batch_size", "num_heads", "num_heads_k", "head_size", "seqlen_q", "seqlen_k", "hexcute", "flash-atten"]
    records = []

    for batch_size in [1]:
        for num_heads in [16, 32]:
            for head_size in [64, 128]:
                for seqlen_q in [512, 1024, 2048, 4096, 8192]:
                    num_heads_k = num_heads
                    seqlen_k = seqlen_q
                    mean_hexcute, mean_flash_atten = main(batch_size, num_heads, num_heads_k, head_size, seqlen_q, seqlen_k, sm_ver)
                    records.append([batch_size, num_heads, num_heads_k, head_size, seqlen_q, seqlen_k, mean_hexcute, mean_flash_atten])
